...
void computePID()
{
    roll = myIMU.get_pitch_rad();
    pitch = myIMU.get_roll_rad();
    pid_pitch.Compute();
    pid_roll.Compute();
    int vx = constrain(vx, -2, 2);  
    int vy = constrain(vy, -2, 2); 
}
...
CytronMD motor1(PWM_DIR, 9, A3);  // PWM 1 = Pin 3, DIR 1 = Pin 4.
CytronMD motor2(PWM_DIR, 10, A2); // PWM 2 = Pin 9, DIR 2 = Pin 10.
CytronMD motor3(PWM_DIR, 11, A1); // PWM 2 = Pin 9, DIR 2 = Pin 10.
